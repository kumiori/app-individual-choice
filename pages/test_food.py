...
def compute_nutrient_contributions(quantities):
    ...
    overall_energy = sum(
        quantities[i] * food_df["Energy (kcal per 100g)"][i] / 100
        for i in range(len(quantities))
    )
    overall_carbs = sum(
        quantities[i] * food_df["Carbohydrate (g per 100g)"][i] / 100
        for i in range(len(quantities))
    )
    overall_protein = sum(
        quantities[i] * food_df["Protein (g per 100g)"][i] / 100
        for i in range(len(quantities))
    )
    overall_fat = sum(
        quantities[i] * food_df["Fat (g per 100g)"][i] / 100
        for i in range(len(quantities))
    )
...
def main():
    ...
    with cols[0]:
        ...
        overall_energy, overall_carbs, overall_protein, overall_fat = (
            compute_nutrient_contributions(quantities)
        )
