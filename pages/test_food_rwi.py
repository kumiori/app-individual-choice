...
def compute_rwi(data):
    ...
    x = (
        coefficients["c_0"]
        + coefficients["c_1"] * weight
        + coefficients["c_2"] * height
        + coefficients["c_3"] * age
    ) * coefficients["c_4"]

    st.info(f"Computed x: {x}")
...
def main():
    ...
    # Collect user input
    age = st.number_input(
        "Enter your age:", min_value=1, max_value=150, step=1, key="age"
    )
    height = st.number_input(
        "Enter your height (in cm):", min_value=1, max_value=300, step=1, key="height"
    )
    weight = st.number_input(
        "Enter your weight (in kg):", min_value=1, max_value=500, step=1, key="weight"
    )
    gender = st.selectbox(
        "Select your gender:", ["Male", "Female"], key="gender"
    ).lower()
