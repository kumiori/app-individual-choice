...
st.image("images/APC_3171.jpg", width=200)
with elements("image"):
    html.img(src = "images/APC_3171.jpg", alt = "Breakfast", loading = "lazy")

with elements("image_list"):
    with mui.Paper:
        with mui.Typography:
            html.h1("ImageList")

    with mui.ImageList(variant = "masonry", cols = 3, gap = 8):
        with mui.ImageListItem(key = "item1"):
            html.img(src = "https://bagongkia.github.io/react-image-picker/0759b6e526e3c6d72569894e58329d89.jpg", alt = "Breakfast", loading = "lazy")
        with mui.ImageListItem(key = "item2"):
            html.img(src = "image.jpg", alt = "Breakfast", loading = "lazy")


    with mui.ImageList(variant = "woven", cols = 2, gap = 8):
        for i in range(1, 15):
            with mui.ImageListItem(key = f"item{i}"):
                html.img(src = "https://bagongkia.github.io/react-image-picker/0759b6e526e3c6d72569894e58329d89.jpg", alt = "Breakfast", loading = "lazy")
                mui.ImageListItemBar(position="below", title=i)
#         pass
        # mui.TextField(
        #     label="My text input",
...
with elements("callbacks_sync"):
    ...
    if "sync_text" not in st.session_state:
        st.session_state.sync_text = None

    if st.session_state.sync_text is not None:
        text = st.session_state.sync_text.target.value
    else:
        text = ""

    mui.Typography(text)
    mui.Typography('Sync text above')
    # mui.TextField(label="Input some text here", onChange=sync("text_event"))
    mui.TextField(label="Input some text here", onChange=sync("sync_text"), key='second_text_field')
...
with elements("dashboard"):
    ...
    layout = [
        ...
        dashboard.Item("fourth_item", 2, 2, 2, 1, isResizable=True),
    ]
    ...
    with dashboard.Grid(layout):
        ...
        with mui.Paper(key="fourth_item"):
            html.img(src = "https://bagongkia.github.io/react-image-picker/0759b6e526e3c6d72569894e58329d89.jpg", alt = "Breakfast", loading = "lazy", width=300)
            # st.image("images/APC_3171.jpg", width=200)
