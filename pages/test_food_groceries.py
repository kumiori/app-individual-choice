...
def add_food_item(food_item, quantity, unit, multiplicity):
    ...
    st.session_state.food_data = pd.concat(
        [st.session_state.food_data, new_row], ignore_index=True
    )
...
if st.download_button(
    label="Download CSV", data=csv, file_name=csv_filename, mime="text/csv"
):
...
if st.button(f"Get Groceries Info {len(st.session_state.food_data)}"):
    ...
    with st.spinner("Fetching nutrient information..."):
        ...
        # data =
        st.session_state.food_data.drop(
            columns=[
...
if st.session_state.food_data.empty:
    st.warning("No nutritional data found, yet.")
    st.stop()
...
st.write(
    f'`{macro_total_df}` grams of nutrients out of {df["Quantity"].sum()/1000:.1f} kgs of mass means {1-macro_total_df/df["Quantity"].sum():.1%} of fiber and water.'
)
...
relative_df_melted = relative_df.melt(
    id_vars=["Food Item"], var_name="Nutrient", value_name="Proportion"
)
